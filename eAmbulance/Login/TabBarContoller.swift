...
private func setupTabBar() {
    ...
    private func createNavController(viewController: UIViewController, title: String, image: String) -> UINavigationController {
        ...
        // Seçilmiş ikon üçün qalın variant
        ...
        // NavBar-ın düzgün göstərilməsi üçün margin-ləri təyin edirik
        ...
        // Bu hissəni əlavə edərək yalnız bir `navigationController` istifadə edin
        nav.isNavigationBarHidden = false  // Yalnız bu tənzimləməni əlavə edin ki, navbar düzgün göstərilsin.
...
func coachMarksController(_ coachMarksController: CoachMarksController, coachMarkAt index: Int) -> CoachMark {
    let sortedTabBarButtons = self.tabBar.subviews
        .filter { $0 is UIControl }  // Burada UIControl-dən olan alt view-ləri alırıq
        .sorted(by: { $0.frame.origin.x < $1.frame.origin.x })  // X sırasına görə sıralanır
    
    guard index < sortedTabBarButtons.count else {
        return coachMarksController.helper.makeCoachMark()  // Əgər index həddindən artıqdırsa, default işarə göstəririk
    }
